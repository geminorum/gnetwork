...
class Mail extends gNetwork\Module
{
	...
	protected function settings_actions( $sub = NULL )
	{
		...
		if ( isset( $_POST['create_log_folder'] ) ) {
			...
			if ( $created = Core\File::putHTAccessDeny( GNETWORK_MAIL_LOG_DIR, TRUE, TRUE ) )
		...
		if ( 'emaillogs' == $sub ) {
			...
			if ( GNETWORK_MAIL_LOG_DIR
				...
				if ( self::isTablelistAction( 'deletelogs_all' ) ) {
					...
					WordPress\Redirect::doReferer( ( FALSE === Core\File::emptyDir( GNETWORK_MAIL_LOG_DIR, TRUE ) ? 'error' : 'purged' ) );
	...
	{
		if ( isset( $args['path'] ) && Core\Text::ends( $args['path'], '.svg' ) )
	...
	public function phpmailer_init( &$phpmailer )
	{
		...
		$phpmailer->Hostname = WordPress\Site::hostname();
	...
	{
		// $this->do_log_mail( $error->get_error_data(), TRUE );
	...
	// TODO: display failed status: `$contents['phpmailer_exception_code']`
	...
	// TODO: migrate to `wp_mail_succeeded` action @since WP 5.9.0
	...
	public function bp_send_email_success( $status, $email )
	{
		$mail = [
			...
			// TODO: use internal
			'message' => \PHPMailer\PHPMailer\PHPMailer::normalizeBreaks( $email->get_content_plaintext( 'replace-tokens' ) ),
		];
