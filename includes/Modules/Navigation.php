...
class Navigation extends gNetwork\Module
{
	...
	public function get_global_menus()
	{
		$list  = [];
		$menus = [
			'GNETWORK_NETWORK_NAVIGATION' => _x( 'Network Global Navigation', 'Modules: Navigation: Global Menu', 'gnetwork-admin' ),
			'GNETWORK_NETWORK_ADMINBAR'   => _x( 'Network Adminbar Navigation', 'Modules: Navigation: Global Menu', 'gnetwork-admin' ),
			'GNETWORK_NETWORK_USERMENU'   => _x( 'Network User Navigation', 'Modules: Navigation: Global Menu', 'gnetwork-admin' ),
			'GNETWORK_NETWORK_EXTRAMENU'  => _x( 'Network Extra Navigation', 'Modules: Navigation: Global Menu', 'gnetwork-admin' ),
		];
	...
	public function load_nav_menus_php()
	{
		...
		$screen->add_help_tab( [
			...
			'title'    => _x( 'Placeholders', 'Modules: Navigation: Help Tab Title', 'gnetwork-admin' ),
			'callback' => [ $this, 'help_tab_placeholders' ],
		] );

		add_meta_box( $this->classs(),
			_x( 'Relative Links', 'Modules: Navigation: Meta Box Title', 'gnetwork-admin' ),
			[ $this, 'do_meta_box' ],
			$screen,
			'side',
			'low' );
	...
	public function do_meta_box()
	{
		...
		$tabs = [
			'general' => [
				'label'       => _x( 'General', 'Modules: Navigation: Tabs', 'gnetwork-admin' ),
				'description' => _x( '<em>General</em> links are relative to the current user and are visible at any time.', 'Modules: Navigation', 'gnetwork-admin' ),
				'pages'       => $this->get_general_pages(),
			],
			'loggedin' => [
				'label'       => _x( 'Logged-In', 'Modules: Navigation: Tabs', 'gnetwork-admin' ),
				'description' => _x( '<em>Logged-In</em> links are relative to the current user, and are not visible to visitors who are not logged in.', 'Modules: Navigation', 'gnetwork-admin' ),
				'pages'       => $this->get_loggedin_pages(),
			],
			'loggedout' => [
				'label'       => _x( 'Logged-Out', 'Modules: Navigation: Tabs', 'gnetwork-admin' ),
				'description' => _x( '<em>Logged-Out</em> links are not visible to users who are logged in.', 'Modules: Navigation', 'gnetwork-admin' ),
		...
		if ( is_multisite() ) {
			$tabs['sites'] = [
				'label'       => _x( 'Sites', 'Modules: Navigation: Tabs', 'gnetwork-admin' ),
				'description' => _x( '<em>Sites</em> on this network within your access.', 'Modules: Navigation', 'gnetwork-admin' ),
	...
	private function add_to_menu_button( $id )
	{
		echo '<p class="button-controls"><span class="add-to-menu">';
			...
			echo ' class="button-secondary submit-add-to-menu right" value="';
				echo esc_attr_x( 'Add to Menu', 'Modules: Navigation', 'gnetwork-admin' );
	...
	public function get_general_pages()
	{
		...
		$items[] = [
			'name' => _x( 'Default Posts Feed', 'Modules: Navigation', 'gnetwork-admin' ),
			'slug' => 'posts_feed',
			'link' => get_feed_link( $default ),
		];

		$items[] = [
			'name' => _x( 'Default Comments Feed', 'Modules: Navigation', 'gnetwork-admin' ),
			'slug' => 'comments_feed',
	...
	public function get_loggedin_pages()
	{
		...
		$items[] = [
			'name' => _x( 'Log Out', 'Modules: Navigation', 'gnetwork-admin' ),
			'slug' => 'logout',
			'link' => $this->get_logout_url(),
		];

		$items[] = [
			'name' => _x( 'Edit Profile', 'Modules: Navigation', 'gnetwork-admin' ),
			'slug' => 'edit_profile',
			'link' => $this->get_edit_profile_url(),
		];

		$items[] = [
			'name' => _x( 'Public Profile', 'Modules: Navigation', 'gnetwork-admin' ),
			'slug' => 'public_profile',
	...
	public function get_loggedout_pages()
	{
		...
		$items[] = [
			'name' => _x( 'Log In', 'Modules: Navigation', 'gnetwork-admin' ),
			'slug' => 'login',
		...
		if ( $register_url = $this->get_register_url() )
			$items[] = [
				'name' => _x( 'Register', 'Modules: Navigation', 'gnetwork-admin' ),
				'slug' => 'register',
				'link' => $register_url,
			];
	...
	public function wp_nav_menu_item_custom_fields_children( $item_id, $item, $depth, $args, $id )
	{
		...
		echo '<fieldset class="description description-wide"><label for="edit-menu-item-children-'.$item_id.'">';
			...
			_ex( 'Include Children as Sub-menu', 'Modules: Navigation', 'gnetwork-admin' );
		echo '</label></fieldset>';
	}
